...
final class WPUserFrontEnd {
    ...
    function wpuser_login() {
        ...
        if (isset($data['wp_user_email_name'])) {
            if (filter_var($data['wp_user_email_name'], FILTER_VALIDATE_EMAIL)) {
                $userInfo = get_user_by_email(sanitize_text_field($data['wp_user_email_name']));
                if (!empty($userInfo->user_login))
                    $creds['user_login'] = $userInfo->user_login;
            }else {
                $creds['user_login'] = sanitize_text_field($data['wp_user_email_name']);
            }
        } else {
            $creds['user_login'] = '';
        }
        if (isset($data['wp_user_password'])) {
            $creds['user_password'] = sanitize_text_field($data['wp_user_password']);
        } else {
            $creds['user_password'] = '';
        }
        ...
        if (isset($wp_user_login_limit_enable) && !empty($wp_user_login_limit_enable)) {
            ...
            if ($confirmResponse == 1) {
                ...
                $result['message'] = $json['ACCESS_DENIED_FOR'] . $wp_user_login_limit_time . $json['MINUTS'];
        ...
        if (!is_wp_error($login_user)) {
            ...
            $result['message'] = $json['INVALID_USER_NAME_OR_PASSWORD'];
            $result['status'] = 'warning';
    ...
    function wpuser_forgot() {
        ...
        $data = array();
        $data = json_decode(file_get_contents("php://input"), true);
        $email = sanitize_text_field($data['wp_user_email']);
    ...
    function wpuser_register() {
      ...
      //  var_dump($data);exit;
        ...
        */
        //end reCaptcha
        if (isset($data['wp_user_email_name']) && !empty($data['wp_user_email_name'])) {
            $username = sanitize_text_field($data['wp_user_email_name']);
        } else {
        ...
        if (isset($data['wp_user_email']) && !empty($data['wp_user_email'])) {
            $email = sanitize_text_field($data['wp_user_email']);
        ...
        if (isset($data['wp_user_password']) && !empty($data['wp_user_password'])) {
            $password = sanitize_text_field($data['wp_user_password']);
        } else {
        ...
        if (isset($data['wp_user_re_password']) && !empty($data['wp_user_re_password'])) {
            ...
            $re_password = sanitize_text_field($data['wp_user_re_password']);
        ...
        if (isset($wp_user_tern_and_condition) && $wp_user_tern_and_condition == 1) {
            if (!(isset($data['wp_user_term_condition']) && !empty($data['wp_user_term_condition']))) {
        ...
        if ($register_user && !is_wp_error($register_user)) {
            ...
            $result['status'] = 'success';
            $args = array($email, $username, $password);
    ...
    function wp_user_action_register_function(&$args) {
        ...
        if (get_option('wp_user_email_admin_register_enable')) {
            ...
            $email_body_text = apply_filters('wp_user_filter_email', get_option('wp_user_email_admin_register_content'), $to, $args[0], null, null, null);
        ...
        if (get_option('wp_user_email_user_register_enable')) {
            ...
            $email_body_text = apply_filters('wp_user_filter_email', get_option('wp_user_email_user_register_content'), $to, $args[1], null, null, null);
            $email_footer_text = 'You\'re receiving this email because you have register on ' . $site_url;
            include(WPUSER_PLUGIN_DIR . 'includes/user/module/template_email_defualt.php');
            $mail = wp_mail($to, $subject, $message, $headers);
