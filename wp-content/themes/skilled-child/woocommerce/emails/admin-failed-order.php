...
if($order->payment_method == 'paypal'){
    printf( __( 'Payment for order #%1$d from %2$s has failed. The order was as follows:', 'woocommerce' ), $order->get_order_number(), $order->get_formatted_billing_full_name() );
}else{
    printf( __( 'Payment for session #%1$d from %2$s has failed. The session was as follows:', 'woocommerce' ), $order->get_order_number(), $order->get_formatted_billing_full_name() );
}
?></p>
 <?php
