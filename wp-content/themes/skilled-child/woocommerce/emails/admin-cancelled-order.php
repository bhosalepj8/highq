...
if($order->payment_method == 'paypal'){
    printf( __( 'The order #%1$d from %2$s has been cancelled. The order was as follows:', 'woocommerce' ), $order->get_order_number(), $order->get_formatted_billing_full_name() );
}else{
    printf( __( 'The session #%1$d from %2$s has been cancelled. The session was as follows:', 'woocommerce' ), $order->get_order_number(), $order->get_formatted_billing_full_name() );
}
?></p>
 <?php
